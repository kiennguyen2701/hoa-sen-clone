...
export default function TourCard({ title, index }) {
  ...
  const image = matchedTour?.image;
  const category = matchedTour?.category || 'Du lịch';
  const duration = matchedTour?.duration || 'Liên hệ';
  const departure = matchedTour?.departure || 'Theo lịch';
  const price = matchedTour?.price || 'Liên hệ';
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-xl bg-[#fcfaf5] p-3">
            ...
            <div className="mt-1 text-[#5f4a33]">{departure}</div>
          </div>
          <div className="rounded-xl bg-[#fcfaf5] p-3">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#9e6d2b]">
              Giá tour
            </div>
            <div className="mt-1 font-bold text-[#744815]">{price}</div>
          </div>
        </div>
