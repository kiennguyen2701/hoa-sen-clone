...
function AppContent() {
  ...
  const isSystemPage =
    ...
    location.pathname.startsWith('/ctv') ||
  ...
  return (
    <div className="min-h-screen bg-[#f7f1e6] text-[#3f2d1d]">
      ...
      {!isSystemPage && (
        <div className="fixed right-3 top-1/2 z-50 flex -translate-y-1/2 flex-col gap-2">
          <a
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg transition hover:scale-105"
            title="Chat Zalo"
          >
            <img
      ...
      {!isSystemPage && <SiteHeader />}
      {!isSystemPage && showHeroSlider && <HeroSlider />}

      <Routes>
        <Route path="/" element={<HomePage />} />
        <Route path="/gioi-thieu" element={<AboutPage />} />
        <Route path="/du-lich-quoc-te" element={<InternationalToursPage />} /><Route path="/du-lich-trong-nuoc" element={<DomesticToursPage />} />
        ...
        <Route
          path="/ctv"
          element={
            <ProtectedRoute allow={['collaborator']}>
              <CollaboratorPage />
            </ProtectedRoute>
          }
        />
